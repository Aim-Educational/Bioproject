...
{
    public partial class group_type : IDataModel
    {
        public bool isDeletable(PlanningContext db)
        {
                        var query0 = from obj in db.alarms
                         where obj.group_type_id == this.group_type_id
                         select obj;

            var query1 = from obj in db.group_action
                         where obj.group_type_id == this.group_type_id
                         select obj;

            var query2 = from obj in db.group_member
                         where obj.group_type_id == this.group_type_id
                         select obj;

            var query3 = from obj in db.group_type
                         where obj.parent_group_type_id == this.group_type_id
                         select obj;
        ...
        {
            var obj = db.group_type.SingleOrDefault(d => d.group_type_id == this.group_type_id);
